...
public class CheckoutService implements ICheckoutService {
	...
	public Checkout buildCheckout(List<String> productIds) {
		...
		for (String id : productIds) {
			...
			// si a este llamado lo meto dentro de un try catch, para que siga intentando,
			// al 5to intento se va a ejecutar el metodo de fallback del circuit breaker
			Product product = productService.getProduct(id);

			System.out.println("Respuesta desde " + product.getInstance());
			total += product.getPrice();
		}
		Checkout checkout = new Checkout("234", "www.digitalhouse.com/checkout?code=234", total.toString(),
				List.of("credit_card"));
